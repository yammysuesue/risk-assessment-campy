...
hh_obs <- readRDS("observational_data_tp2_processed_update.RDS")
hh_obs2 <- hh_obs
...
hh_obs$start <- as.POSIXct(hh_obs$Start.datetime, format="%m/%d/%Y %H:%M:%S", tz="UTC")
hh_obs$end   <- as.POSIXct(hh_obs$End.datetime, format="%m/%d/%Y %H:%M:%S", tz="UTC")
