...
# check font in windows
windowsFonts()
windowsFonts(Arial = windowsFont("TT Arial"))

plot(age1,vertex.label.family = "Arial",vertex.label.cex=0.75,edge.width=edge.wt,edge.color=edge.col.list[edge.wt*10-9])
abline(h = -0.05)
mtext("off homestead",2,at=-0.7)
mtext("on homestead",2,at=0.5)

fileps <- function(name) paste(ver,".TimePoint",agecat,"_iter_",num.sim,"_",Sys.Date(),".eps",sep="");
