...
source_spec <- data.frame(
  source = c("HP", "HR"),
  source_label = c("mother handrinse", "infant handrinse"),
  value_col = c("dna_via", "dna_via"),
  cens_col = c("cens_via", "cens_via"),
  lod_col = c("dna_via_lod", "dna_via_lod"),
...
build_bvn_overlay_df <- function(source_spec, n_draws = 5000) {
  ...
  out <- lapply(source_spec$source, function(src) {
...
overview_plot <- make_source_plot(
  ...
  lod_data = lod_max_df,
  title_text = "fitdistrplus left-censored normal fits with HP/HR BVN overlay",
  facet = TRUE
)
...
ggsave(
  filename = file.path(getwd(), "fitdistrplus_leftcens_HP_HR_overview.png"),
  plot = overview_plot,
  width = 8,
  height = 4.5,
  dpi = 300
...
write.csv(
  fit_summary_df,
  file = file.path(getwd(), "fitdistrplus_leftcens_HP_HR_summary.csv"),
  row.names = FALSE
)
...
saveRDS(
  fit_object_list,
  file = file.path(getwd(), "fitdistrplus_leftcens_HP_HR_fit_objects.rds")
)
